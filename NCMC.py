...
class NCMC():
    """
    Nearest Class Multiple Cnetroids (NCMC) v0.2
    ...
    def saveModel(self, file_name=""):
        if file_name == "":
            file_name = "./NCMC_model_" + strftime("%Y_%m_%d") + ".pickle"
    ...
    def saveWeights(self, file_name=""):
        if file_name == "":
            np.save("./NCMC_weights_" + strftime("%Y_%m_%d"), self.W_output, allow_pickle=True)
        else:
    ...
    def fit(self, X, y, X_val, y_val, semantic_dim=500, learning_rate=0.001, init=None, num_centroids=3,
            reg=0.01, max_iters=100, batch_size=256, decay=0.8, num_decay=5, stop_tol=0.001, num_stop_tol=30):
        """
        ...
        for i in range(0, max_iters):
            ...
            # compute gradient 1
            for idx in range(0, x_batch.shape[0]):
                sp = y_batch[idx] * self.num_centroids
                ep = (y_batch[idx] + 1) * self.num_centroids
                sum_class_prob = np.sum(alpha[idx, sp:ep])

                if sum_class_prob == 0: sum_class_prob = 1
                weighted_class_prob = alpha[idx, sp:ep] / sum_class_prob
                alpha[idx, sp:ep] -= weighted_class_prob
    ...
    def __initCentroids(self, X, y):
        ...
        # output numpy array
        centroids = np.empty([num_classes * self.num_centroids, D])
        
        start_time = time()
        for i, class_i in enumerate(classes):
            Xc = X[y == class_i]
            kmeans = KMeans(n_clusters=self.num_centroids)
            kmeans.fit(Xc) # K-means clustering among same class
            centroids[i*self.num_centroids:(i+1)*self.num_centroids, :] = kmeans.cluster_centers_
    ...
    def __nextBatch(self, X, y, batch_size, idx):
        """
        Sampling batch data 
        """
    ...
    def __softmaxProbLoss(self, data, centroids, label=None, eps=0.000001):
        ...
        N = data.shape[0]
        softmax_prob_class = np.empty(N)
        
        softmax_prob = self.__softmax(-self.__squaredDist(data, centroids))
        label_pred = np.argmax(softmax_prob, axis=1) / self.num_centroids
        
        label = label.astype(self.int_type)
        label_pred = label_pred.astype(self.int_type)
        
        if label is None:
            return softmax_prob, label_pred, 0
        
        for i in range(0, N):
            sp = label[i] * self.num_centroids
            ep = (label[i] + 1) * self.num_centroids
            softmax_prob_class[i] = np.sum(softmax_prob[i, sp:ep])
    ...
    def predictDist(self, X, y=None, new_class=999):
        ...
        # predict
        y_pred = np.argmin(dist, axis=1) / self.num_centroids
        y_pred = y_pred.astype(self.int_type)
